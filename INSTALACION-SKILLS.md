...
del repo — valida cada `SKILL.md` con `validate_skill.py` y deja los 63 zips
(más el scaffold `tu-nombre-de-skill`) en `dist/` (carpeta ignorada por git,
se regenera cuando quieras).

```bash
./scripts/package_skills_for_upload.sh                      # las 63
...
las 63 skills viven en `.claude/skills/` de este repositorio y están
